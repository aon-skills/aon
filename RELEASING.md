...
- The TypeScript runtime artifact is the npm package `@agentoffernetwork/skill`.
...
- the published npm skill version changes
...
- `Publish GitHub Release` is a manual Actions workflow. Run it with a tag that matches the published npm version when the wrapper repo needs a release snapshot.
...
```bash
PUBLISH_MSG="docs(aon): sync from monorepo" bash sdk/publish_skills.sh aon-skills aon
```

## Versioning Guidance

- Use the npm package version as the GitHub release tag version.
